...
const Booking = () => {
  ...
  const handleReturnPassengerChanges = (index, field, value) => {
    setPassengers((prevReturnPassengers) => {
      const updatedReturnPassengers = [...prevReturnPassengers];
  
      if (field === 'name') {
        // Validate the name as a string
        if (/^[a-zA-Z]*$/.test(value)) {
          updatedReturnPassengers[index] = { ...updatedReturnPassengers[index], [field]: value };
        } else {
          // Optionally, handle invalid name input (e.g., show an error message).
        }
      } else if (field === 'age') {
        ...
        if (!isNaN(ageValue) && ageValue >= 0 && ageValue <= 200) {
          updatedReturnPassengers[index] = { ...updatedReturnPassengers[index], [field]: ageValue };
        } else {
          // Optionally, handle invalid age input (e.g., show an error message).
        }
      } else if (field === 'gender') {
      ...
      return updatedReturnPassengers;
    });
  };
